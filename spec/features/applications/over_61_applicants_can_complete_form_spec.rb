...
RSpec.feature 'Completing the application details', type: :feature do
  ...
  context 'where the applicant is over 61', js: true do
    ...
    scenario 'shows the proper summary page' do
      expect(page).to have_xpath('//div[contains(@class,"callout")][contains(@class, "callout-part")]/h3[@class="heading-large"]', text: 'The applicant must pay £40 towards the fee')
    end
  end
  ...
  context 'when user returns and selects savings threshold exceeded', js: true do
    ...
    scenario 'shows the proper summary page' do
      expect(page).to have_xpath('//div[contains(@class,"callout")][contains(@class, "callout-none")]/h3[@class="heading-large"]', text: '✗ Not eligible for help with fees')
    end
  end
end
