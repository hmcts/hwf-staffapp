...
RSpec.describe Applications::Process::IncomeKindPartnersController do
  let(:user)          { create(:user) }
  let(:application) { build_stubbed(:application, :applicant_full, office: user.office, married: married) }
  let(:income_kind_form) { instance_double(Forms::Application::IncomeKindPartner) }
  let(:married) { false }

  before do
    sign_in user
    allow(Application).to receive(:find).with(application.id.to_s).and_return(application)
    allow(Forms::Application::IncomeKindPartner).to receive(:new).with(application).and_return(income_kind_form)
  end
  ...
  describe 'PUT #income_kind_save' do
    let(:expected_params) { { income_kind_partner: ['test'] } }
