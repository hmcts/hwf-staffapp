...
RSpec.describe RawDataExportJob do
  ...
  describe '#perform' do
    it "run the export query" do
      ...
      expect(Views::Reports::RawDataExport).to have_received(:new).with('1', '2', nil)
    end
    ...
    context 'notifications' do
      ...
      it { expect(app_insight).to have_received(:track_event).with("Running RawDataExport start at #{Time.zone.now.to_fs(:db)}") }
      it { expect(app_insight).to have_received(:track_event).with("Running RawDataExport end at #{Time.zone.now.to_fs(:db)}") }
