...
Then("I should see my reasons for evidence on the summary page") do
  ...
  expect(summary_page.content.summary_section[0].list_row[1].text).to eq 'Ready to process No Change Ready to process'
  expect(summary_page.content.summary_section[0].list_row[2].text).to eq 'Reasons Requested sources not provided, Wrong type provided, Unreadable or illegible, Pages missing, Cannot identify applicant, Wrong date range Change Reasons'
end

Then("I should see my reason for evidence on the summary page") do
  expect(summary_page.content.summary_section[0]).to have_evidence_header
  expect(summary_page.content.summary_section[0].list_row[1].text).to eq 'Ready to process No Change Ready to process'
  expect(summary_page.content.summary_section[0].list_row[2].text).to eq 'Reason Requested sources not provided Change Reason'
end
