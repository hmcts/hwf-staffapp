...
When("I change my details") do
  ...
  expect(profile_page.content.notice.text).to have_text 'User updated. We have sent an email with a confirmation link'
end

Then("I can see my profile has been changed") do
  expect(profile_page.content.profile.text).to have_text 'Test User'
end
