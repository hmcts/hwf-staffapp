...
And("I can see my processed application") do
  expect(process_online_application_page.content.last_application[1].text).to have_text 'processed Peter Smith'
end
...
Then("I see digital Emergency advice") do
  expect(application_details_digital_page.content.guidance.guidance_header[2].text).to eq 'Emergency cases'
  expect(application_details_digital_page.content.guidance.guidance_text[5].text).to eq 'An emergency case is one where delay risks harm to the applicant or to the applicant’s case.'
end
...
Then("I see digital examples of emergency cases") do
  ...
  expect(application_details_digital_page.content.guidance.guidance_link[3]['href']).to include 'sourcedoc=%7BB62AF5DB-DF50-4415-A261-A4598E61B298%7D'
end
