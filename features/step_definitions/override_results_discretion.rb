...
Then("The results should show the application passed saving and investments by manager's discretion") do
  expect(confirmation_page.content.summary_list_row[0].text).to have_text 'Savings and investments'
  expect(confirmation_page.content.summary_list_row[0].text).to have_text '✓ Passed (by manager\'s decision)'
end

Then("The results should show the application passed income by manager's discretion") do
  expect(confirmation_page.content.summary_list_row[1].text).to have_text 'Income'
  expect(confirmation_page.content.summary_list_row[1].text).to have_text '✓ Passed (by manager\'s decision)'
end

Then("The results should show the application passed benefits by manager's discretion") do
  expect(confirmation_page.content.summary_list_row[1].text).to have_text 'Benefits'
  expect(confirmation_page.content.summary_list_row[1].text).to have_text '✓ Passed (by manager\'s decision)'
end
...
Then("I should see that the application fails because of income") do
  expect(confirmation_page.content).to have_ineligible
  expect(confirmation_page.content.summary_list_row[1].text).to have_text 'Income'
  expect(confirmation_page.content.summary_list_row[1].text).to have_text '✗ Failed'
end
