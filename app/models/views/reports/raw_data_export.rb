...
module Views
  module Reports
    class RawDataExport < ReportBase
      ...
      # spelling matches every other export (see ColumnLabels).
      L = ColumnLabels
      FIELDS = {
        id: L.fetch(:id),
        name: L.fetch(:office),
        reference: L.fetch(:reference),
        jurisdiction: L.fetch(:jurisdiction),
        sop_code: L.fetch(:sop_code),
        fee: L.fetch(:fee),
        fee_code: L.fetch(:fee_code),
        claim_amount: L.fetch(:claim_amount),
        fee_population: L.fetch(:fee_population),
        estimated_amount_to_pay: L.fetch(:estimated_amount_to_pay),
        estimated_cost: L.fetch(:estimated_cost),
        application_type: L.fetch(:application_type),
        form_name: L.fetch(:form),
        refund: L.fetch(:refund),
        emergency: L.fetch(:emergency),
        income: L.fetch(:pre_evidence_income),
        check_income: L.fetch(:post_evidence_income),
        income_period: L.fetch(:income_period),
        reg_number: L.fetch(:ho_ni_number),
        children: L.fetch(:children),
        children_age_band_one: L.fetch(:age_band_under_14),
        children_age_band_two: L.fetch(:age_band_14_plus),
        married: L.fetch(:married),
        over_66: L.fetch(:pension_age),
        decision: L.fetch(:decision),
        saving_failed: L.fetch(:failed_on_savings),
        final_amount_to_pay: L.fetch(:final_amount_to_pay),
        decision_cost: L.fetch(:departmental_cost),
        ...
        part_payment_outcome: L.fetch(:part_payment_outcome),
        pp_outcome: L.fetch(:pp_outcome),
        low_income_declared: L.fetch(:low_income_declared),
        case_number: L.fetch(:case_number),
        postcode: L.fetch(:postcode),
        date_of_birth: L.fetch(:date_of_birth),
        date_received: L.fetch(:date_received),
        decision_date: L.fetch(:decision_date),
        date_fee_paid: L.fetch(:date_paid),
        application_processed_date: L.fetch(:application_processed_date),
        manual_process_date: L.fetch(:manual_evidence_processed_date),
        date_submitted_online: L.fetch(:date_submitted_online),
        statement_signed_by: L.fetch(:statement_signed_by),
        partner_ni: L.fetch(:partner_ni_entered),
        partner_name: L.fetch(:partner_name_entered),
        calculation_scheme: L.fetch(:hwf_scheme),
        db_evidence_check_type: L.fetch(:db_evidence_check_type),
        db_income_check_type: L.fetch(:db_income_check_type),
        hmrc_total_income: L.fetch(:hmrc_total_income),
        ev_check_outcome: L.fetch(:evidence_check_outcome),
        evidence_check_type: L.fetch(:evidence_check_type),
        hmrc_response: L.fetch(:hmrc_response),
        hmrc_errors: L.fetch(:hmrc_errors),
