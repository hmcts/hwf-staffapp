namespace :reports do
  ...
  desc 'Generate raw digital data extract for given time frame'
  task :raw_digital_data_extract, [:start, :end] => :environment do |_t, args|
    ...
    if from < to
      ...
      report = Views::Reports::RawDigitalDataExport.new(start_date, end_date)
      report.to_zip
      puts "Report file generated raw_data-#{from}-#{to}.csv.zip"
    else
      puts 'Please call the report with following format:'
      puts 'rake "reports:raw_digital_data_extract[2021-01-01, 2022-12-31]"'
    end
  end
end
