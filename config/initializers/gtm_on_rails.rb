GtmOnRails.configure do |config|
  ...
  config.container_id = Settings.analytics.id
  ...
  # Be careful using this, if you enable this option, controller and action name
  # output in html source code.
  config.send_controller_and_action_in_data_layer = false

  # If somthing error occurred when output tags, subsequent tag's output is stopped
  # and run subsequent processing.
  ...
  config.ecommerce_default_currency = 'GBP'
end
